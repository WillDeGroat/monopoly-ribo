...
class NBInteractionEngine:
    ...
    def fit(self, dataset: Any) -> EngineFit:
        ...
        if exposure.isna().any():
            raise ValueError('Effective exposures are missing for one or more samples.')

        if not np.all(np.isfinite(exposure.to_numpy())) or (exposure <= 0.0).any():
            raise ValueError('Effective exposures must contain only finite positive values.')
        ...
        if dataset.abundance_fraction is not None:
            ...
            abundance_normalized_counts = normalized_counts.loc[abundance_mask].copy()
        ...
        return EngineFit(
            ...
            metadata = {
                'abundance_design': 'abundance_fraction ~ condition + covariates',
                'abundance_covariance': 'HC3 heteroskedasticity-robust covariance',
                'redistribution_design': (
                ...
                'model_family': 'Gaussian linear models on log2 exposure-normalized counts',
...
def nb_contrast(fit: EngineFit, dataset: Any, contrast: FractionContrast) -> pd.DataFrame:
    if contrast.kind == 'abundance':
        ...
        if model_name not in fit.models:
            raise NonEstimableContrastError(
                f'Contrast {contrast.label!r} requires a configured abundance fraction.'
            )

        if contrast.fraction != dataset.abundance_fraction:
            raise NonEstimableContrastError(
                f'Contrast {contrast.label!r} is not an abundance contrast for the configured '
                f'abundance fraction {dataset.abundance_fraction!r}.'
            )
    ...
    _validate_estimability(model, contrast, contrast_vector)

    coefficients = model['coef']
    effects = coefficients.to_numpy() @ contrast_vector
    contrast_variance = _contrast_variance(model, contrast_vector)
    ...
    statistics[valid_statistics] = (
        effects[valid_statistics] / standard_errors[valid_statistics]
    )
    ...
    base_mean = model['base_mean'].reindex(coefficients.index)
    informative_counts = (
        model['counts']
...
def _fit_linear_model(
    ...
    if not design.index.equals(response.index):
        raise ValueError('The design matrix and response matrix must have matching sample indices.')
    ...
    if design_rank < design_array.shape[1]:
        raise ValueError('The fitted design matrix must have full column rank.')

    residual_degrees_freedom = design_array.shape[0] - design_rank

    if residual_degrees_freedom <= 0:
        raise ValueError('The fitted model requires positive residual degrees of freedom.')

    fitted_values = design_array @ coefficients
    residuals = response_array - fitted_values
    bread = np.linalg.inv(design_array.T @ design_array)
    estimability_projection = np.linalg.pinv(design_array) @ design_array
    leverage = np.sum((design_array @ bread) * design_array, axis = 1)
    ...
    model: dict[str, Any] = {
        ...
        'n_subjects': int(metadata[subject].astype(str).nunique()),
        'covariance_type': covariance_type
    }

    if covariance_type == 'hc3':
        if np.any(leverage >= 1.0 - LEVERAGE_TOLERANCE):
            raise ValueError(
                'The abundance model has leverage values too close to one for HC3 inference.'
            )

        model['leverage'] = leverage
        model['test_df'] = int(residual_degrees_freedom)
        return model

    if covariance_type != 'cluster':
        raise ValueError(f'Unsupported covariance type {covariance_type!r}.')

    subject_values = metadata[subject].astype(str)
    cluster_codes, cluster_levels = pd.factorize(subject_values, sort = True)
...
def _contrast_orientation(fit: EngineFit, contrast: FractionContrast) -> float:
    fitted_case = fit.models['case']
    fitted_control = fit.models['control']

    if contrast.case == fitted_case and contrast.control == fitted_control:
        return 1.0

    if contrast.case == fitted_control and contrast.control == fitted_case:
        return -1.0

    raise NonEstimableContrastError(
        f'Contrast {contrast.label!r} uses condition levels that do not match the fitted '
        f'case {fitted_case!r} and control {fitted_control!r}.'
    )
...
def _abundance_contrast_vector(
    ...
    condition_column = f'condition_{fit.models["case"]}'

    if condition_column not in column_indices:
        raise NonEstimableContrastError(
            f'Contrast {contrast.label!r} is not estimable because condition column '
            f'{condition_column!r} is absent from the abundance model.'
        )

    contrast_vector = np.zeros(len(columns), dtype = float)
    contrast_vector[column_indices[condition_column]] = orientation
...
def _redistribution_contrast_vector(
    ...
    known_fractions = set(dataset.fraction_order)

    for fraction in [contrast.numerator, contrast.denominator]:
        if fraction not in known_fractions:
            raise NonEstimableContrastError(
                f'Contrast {contrast.label!r} refers to unknown fraction {fraction!r}.'
            )
    ...
    for fraction, coefficient in [
        ...
        interaction_column = f'{condition_column}:fraction_{fraction}'

        if interaction_column not in column_indices:
            raise NonEstimableContrastError(
                f'Contrast {contrast.label!r} is not estimable because interaction column '
                f'{interaction_column!r} is absent from the redistribution model.'
            )

        contrast_vector[column_indices[interaction_column]] += (
            orientation * coefficient
        )

    if not np.any(contrast_vector):
        raise NonEstimableContrastError(
            f'Contrast {contrast.label!r} has an empty redistribution contrast vector.'
        )

    return contrast_vector


def _validate_estimability(model: dict[str, Any], contrast: FractionContrast, contrast_vector: np.ndarray) -> None:
    projection = model['estimability_projection']
...
def _contrast_variance(model: dict[str, Any], contrast_vector: np.ndarray) -> np.ndarray:
    design_array = model['design_array']
    bread = model['bread']
    residuals = model['residuals']
    influence_weights = design_array @ bread @ contrast_vector

    if model['covariance_type'] == 'hc3':
        leverage_adjustment = 1.0 - model['leverage']
        adjusted_residuals = residuals / leverage_adjustment[:, np.newaxis]
        return np.sum(
            np.square(influence_weights[:, np.newaxis] * adjusted_residuals),
            axis = 0
        )

    cluster_scores = []

    for cluster_code in np.unique(model['cluster_codes']):
        cluster_mask = model['cluster_codes'] == cluster_code
        cluster_score = np.sum(
            influence_weights[cluster_mask, np.newaxis]
            * residuals[cluster_mask],
            axis = 0
        )
        cluster_scores.append(cluster_score)

    return model['cluster_correction'] * np.sum(
        np.square(np.vstack(cluster_scores)),
        axis = 0
    )


def _validate_counts(counts: pd.DataFrame, metadata: pd.DataFrame) -> None:
    if counts.empty:
